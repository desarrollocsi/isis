...
export class ReclamosRegistrarComponent implements OnInit, OnDestroy {
	...
	getDataReclamo() {
		this.IS._dataReclamo
			.pipe(tap((_) => {
				...
				this.nrodocp_valid()
				this.nrodoc_valid()
				if (data.nro_documento == data.nro_documento_p && data.tipo_documento == data.tipo_documento_p) {
					this.afectada = true
				} else {
					this.afectada = false
				}
	...
	getEtapaResultado(estado: number) {
		var etp, res, x
		if (estado == 1) { etp = [3]; res = [1, 2, 3, 4, 5]; this.campos.etapa.setValue(3) }
		if (estado == 2) { etp = [1, 2, 3]; res = [0]; this.campos.resultado.setValue(0) }
		if (estado == 3) { etp = []; res = [0]; this.campos.resultado.setValue(0) }
		if (estado == 4 || estado == 5) { etp = []; res = [] }
		if (estado == 6) { etp = [4]; res = [1, 2, 3, 4, 5]; this.campos.etapa.setValue(4) }
